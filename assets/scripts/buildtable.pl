...
my $sipnum = 0;
while (++$sipnum <= $topsip) {
	my $fn = sprintf "sip-%04d.mediawiki", $sipnum;
	-e $fn || next;
	open my $F, "<$fn";
	...
	while (<$F>) {
		...
		if ($field eq 'SIP') {
			die "$fn claims to be SIP $val" if $val ne $sipnum;
		} elsif ($field eq 'Title') {
			$title = $val;
			my $title_len = length($title);
			die "$fn has too-long TItle ($title_len > 44 char max)" if $title_len > 44 and not exists $TolerateTitleTooLong{$sipnum};
		} elsif ($field eq 'Author') {
			$val =~ m/^(\S[^<@>]*\S) \<([^@>]*\@[\w.]+\.\w+)\>$/ or die "Malformed Author line in $fn";
			my ($authorname, $authoremail) = ($1, $2);
			$authoremail =~ s/(?<=\D)$sipnum(?=\D)/<SIPNUM>/g;
			...
			if (not $found{$field}) {
				die "Unacceptable license $val in $fn" unless exists $AcceptableLicenses{$val} or ($val eq 'PD' and exists $GrandfatheredPD{$sipnum});
			}
		} elsif ($field eq 'Comments-URI') {
			if (not $found{'Comments-URI'}) {
				my $first_comments_uri = sprintf('https://github.com/sirenmarkets/sips/wiki/Comments', $sipnum);
	...
	if (not $found{License}) {
		die "Missing License in $fn" unless exists $TolerateMissingLicense{$sipnum};
	}
